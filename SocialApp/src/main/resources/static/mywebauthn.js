...
(() => {
  ...
  async function authenticate(headers, contextPath, useConditionalMediation) {
    ...
    //!!!return authenticationResponse.redirectUrl; // по умолчанию
	return `/profile/`; //указываем свой адрес, куда будет переход при успешной аутентификации
  }
  ...
  async function setupRegistration(headers, contextPath, ui) {
    ...
    ui.getRegisterButton().addEventListener("click", async () => {
      ...
      try {
        ...
        window.location.href = `${contextPath}/settings/?success`; //редирект при успешной регистрации токена
      } catch (err) {
    ...
    ui.getDeleteForms().forEach(
      (form) => form.addEventListener("submit", async function(e) {
        ...
        try {
          ...
          window.location.href = `${contextPath}/settings/?success`; //редирект при успешном удалении токена
        } catch (err) {
          setError(ui, err.message);
        }
      })
